...
func TestServePluginAuthURLPassesQueryParamsAsMetadata(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	ctx, _ := gin.CreateTestContext(rec)
	req := httptest.NewRequest(http.MethodGet, "/v0/management/custom-sso-auth-url?idc_region=eu-west-1&idc_start_url=https%3A%2F%2Fsso.example.com&scopes=read&scopes=write&token=secret&is_webui=true", nil)
	ctx.Request = req
	...
	for _, key := range []string{"token", "is_webui"} {
		if _, exists := capturedReq.Metadata[key]; exists {
			t.Fatalf("host-only query parameter %q reached plugin", key)
		}
	}
