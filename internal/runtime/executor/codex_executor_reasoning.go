...
func applyCodexReasoningReplayCacheRequired(ctx context.Context, from sdktranslator.Format, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, body []byte) ([]byte, codexReasoningReplayScope, error) {
	...
	return applyCodexReasoningReplayScopeRequired(ctx, scope, body)
}

func applyCodexReasoningReplayCacheForAuthRequired(ctx context.Context, auth *cliproxyauth.Auth, from sdktranslator.Format, req cliproxyexecutor.Request, opts cliproxyexecutor.Options, body []byte) ([]byte, codexReasoningReplayScope, error) {
	scope := codexReasoningReplayScopeForAuth(codexReasoningReplayScopeFromRequest(ctx, from, req, opts, body), auth)
	return applyCodexReasoningReplayScopeRequired(ctx, scope, body)
}

func applyCodexReasoningReplayScopeRequired(ctx context.Context, scope codexReasoningReplayScope, body []byte) ([]byte, codexReasoningReplayScope, error) {
	if !scope.valid() {
...
func cacheCodexReasoningReplayFromCompleted(scope codexReasoningReplayScope, completedData []byte) {
	cacheCodexReasoningReplayFromCompletedWithContext(context.Background(), scope, completedData)
}

func cacheCodexReasoningReplayFromCompletedWithContext(ctx context.Context, scope codexReasoningReplayScope, completedData []byte) {
	if !scope.valid() {
		return
	}
	...
	if scope.generation == "" {
		internalcache.AppendCodexReasoningReplayItemsBestEffort(ctx, scope.modelName, scope.sessionKey, items)
		return
	}
